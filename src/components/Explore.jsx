...
export default function Explore() {
    ...
    const CardList = exploreWorlds.map((item,key)=>{
        return(
            <motion.div className={`px-1 z-20 ${key===num?"w-4/12":"w-2/12 relative"}  overflow-hidden cursor-pointer rounded-2xl`} variants={fadeIn("right","spring",(key+1)*0.5,0.5)} initial="hidden" animate={control} key={key} onClick={()=>{setnum(key)}}>
    ...
    return (
      <motion.section className='cont -mt-8 pb-28 relative' variants={fadeUp} initial="initial" animate="animate">
      <TopSection/>
      <div className='flex items-center mt-10' ref={ref}>
            {CardList}
      </div>
